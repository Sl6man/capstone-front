...
async function Page() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/media/neighborhood`, {
    cache: "no-store",
  })
...
//--------------------------------------------------------
  const res1 = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/media/neighborhood/duration`, {
    cache: "no-store",
  })
...
//--------------------------------------------------------
const res2 = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/media/neighborhood/duplicates`, {
  cache: "no-store",
})
...
const res4 = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/media/snaps/weekdays`, {
  cache: "no-store",
});
if (!res4.ok) {
  ...
  return (
    <div className='flex-col items-center m-6 overflow-auto overflow-x-hidden'>
      ...
      <div className='flex justify-start gap-10 my-4 mx-10'>
        <KpiCard background={"yellow-300"} label='Media Collected' icon={<IoMdPhotos/>} value={total_snaps} textStyle='white'/>
        <KpiCard background={"white"} label='Top Neighborhood' icon={<GrLocationPin />} value={top_neighborhood} textStyle='yellow-400'/>
        <KpiCard background={"gray-500"} label='Silent Neighborhood' icon={<MdLocationDisabled />} value={lowest_neighborhood} textStyle='gray-400'/>
      </div>
    ...
    <div className='flex justify-start  my-6 w-full mx-10 gap-10 '>
      ...
      <WeekdaySnapsChart data={weekDaysData}/>
        ...
        <div className='flex-col items-start w-[540] '>
